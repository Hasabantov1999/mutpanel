...
export default function CreateMutPage() {
    ...
    const updateManuelYatirim = (id: string, field: "isim" | "miktar", value: string) => {
        setManuelYatirimlar(
            manuelYatirimlar.map((m) => (m.id === id ? { ...m, [field]: value } : m))
        )
    }

    const updateManuelCekim = (id: string, field: "isim" | "miktar", value: string) => {
        setManuelCekimler(
            manuelCekimler.map((m) => (m.id === id ? { ...m, [field]: value } : m))
        )
    }

    const removeManuelYatirim = (id: string) => {
        setManuelYatirimlar(manuelYatirimlar.filter((m) => m.id !== id))
    }

    const removeManuelCekim = (id: string) => {
    ...
    return (
        <div className="create-mut-page">
            <div className="page-header">
                <h1 className="page-title">Yeni MUT Kaydı</h1>
            </div>
                            ...
                            {manuelYatirimlar.map((entry) => (
                                <div key={entry.id} className="manuel-entry-row">
                                    <input
                                        type="text"
                                        className="form-input"
                                        placeholder="İsim (örn: Ahmet Yılmaz)"
                                        value={entry.isim}
                                        onChange={(e) =>
                            ...
                            {manuelCekimler.map((entry) => (
                                <div key={entry.id} className="manuel-entry-row">
                                    <input
                                        type="text"
                                        className="form-input"
                                        placeholder="İsim (örn: Mehmet Demir)"
                                        value={entry.isim}
                                        onChange={(e) =>
